...
\todo{Content to fill in (from v5 + Joseph Section 3 intro). The spectral
theorem and the eigenvalue characterization of PD/PSD are now in
Section~\ref{subsec:linalg}, so this subsection focuses purely on the
sample object.
...
- Matrix form: \(\Sigmahat_n = \frac{1}{n} \mat X \mat X\transpose\) for
  the data matrix \(\mat X \in \R^{p \times n}\).
- Numerical examples (from v5 \S5.3): exam scores p=2 case, algebraic
  perfectly correlated case, mean-zero case.
- Show that \(\Sigmahat_n\) is symmetric and PSD (one-line application of
  the same \(\Var(\vect{v}\transpose \vect{X})\) identity), so all its
  eigenvalues are nonnegative by Theorem~\ref{thm:pd-psd-eigen}.
- Remark: \(\Sigmahat_n\) is the empirical analogue of \(\Sigma\); the
  whole point of the rest of the section is to compare them.}
