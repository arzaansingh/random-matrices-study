...
\multicolumn{3}{@{}l}{\textit{Spectral clustering (Section 4)}} \\
\midrule
\(K,\ k(i)\) & Number of classes; class of observation \(i\) & \S4.1 \\
\(\vect{\mu}_k,\ \mat{M}\) & Mean of class \(k\); means matrix (\(p \times K\)) & \S4.1 \\
\(\mat{W},\ \mat{P} = \mat{M}\mat{J}\transpose\) & Noise matrix; low-rank signal matrix & \S4.1 \\
\(\mat{J}\) & Assignment matrix (\(\{0,1\}^{n \times K}\)) & \S4.1 \\
\(n_k,\ \mat{D} = \diag(n_1,\dots,n_K)\) & Group sizes; size diagonal & \S4.1 \\
\(\mat{L} = \mat{M}\mat{D}^{1/2},\ \mat{V} = \mat{J}\mat{D}^{-1/2}\) & Scaled means; normalized-indicator matrix & \S4.1 \\
\(\vect{v}_k\) & Normalized indicator / population eigenvector & \S4.1 \\
\(\mat{G} = \tfrac1p \mat{X}\transpose\mat{X}\) & Gram (similarity) matrix (\(n \times n\)) & \S4.1 \\
\(\ell_k,\ \ell\) & Signal-to-noise ratio (eigenvalue of \(\tfrac1n\mat{L}\transpose\mat{L}\)) & \S4.1 \\
\(\xi_k,\ \zeta_k\) & Limiting Gram outlier location; alignment & \S4.1 \\
\(\widehat{\vect{v}}_k,\ \widehat{\zeta}\) & \(k\)-th Gram eigenvector; estimated alignment & \S4.1--4.2 \\
\midrule
